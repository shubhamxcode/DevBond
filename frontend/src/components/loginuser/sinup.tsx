...
const RegisterUser = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setmessage("oops there is an error during registration");
    }
  };
    ...
    <div>
      <div className="flex items-center justify-center min-h-screen bg-gray-900">
        <div className="w-full max-w-md bg-gray-800 shadow-lg rounded-lg p-8">
          ...
          <div className="flex text-white text-xl gap-x-2">
            <h1>Aleady have an account?</h1>
            <Link to='/login' className="hover:underline hover:text-green-600 hover: cursor-pointer">Login</Link>
          </div>
        </div>
      </div>
    </div>
